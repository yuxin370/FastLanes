...
bool images_approx_equal(const ImageRGB& img1, const std::vector<std::vector<std::vector<uint8_t>>>& img2_rgb) {
    ...
    if (img2_rgb[0].size() != H || img2_rgb[0][0].size() != W) {
        std::cerr << "Size mismatch! original: " << W << "x" << H
                  << " reconstructed: " << (img2_rgb[0].empty() ? 0 : img2_rgb[0][0].size())
                  << "x" << img2_rgb[0].size() << std::endl;
        return false;
    }
    ...
    for (size_t h = 0; h < H; ++h) {
        for (size_t w = 0; w < W; ++w) {
            ...
            const int tol = 5;
            if (std::abs((int)r_orig - (int)r_rec) > tol ||
                std::abs((int)g_orig - (int)g_rec) > tol ||
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "Usage: ./jpeg_loader_demo_dir <jpeg_directory> <fls_output_dir>" << std::endl;
        return 1;
    }
    ...
    for (const auto& jp : jpeg_paths) {
        try {
            ...
            az_printer::bold_red_cout << "-- Warning: failed to load original JPEG '" << jp << "': " << ex.what() << std::endl;
    ...
    try {
        ...
        auto reconstructed_collection = fls_reader->to_rgb((fls_dir / "header.meta").c_str());
        ...
        for (size_t idx = 0; idx < N_rec; ++idx) {
            ...
            if (H > 0 && W > 0) {
                std::cout << "   First pixel: ("
                          << static_cast<int>(rec[0][0][0]) << ", "
                          << static_cast<int>(rec[1][0][0]) << ", "
                          << static_cast<int>(rec[2][0][0]) << ")" << std::endl;
            }
            ...
            if (idx < originals.size() && originals[idx].width > 0 && originals[idx].height > 0) {
                try {
                    ...
                    az_printer::bold_red_cout << "-- Error comparing images for index " << idx << ": " << ex.what() << std::endl;
                }
            } else {
