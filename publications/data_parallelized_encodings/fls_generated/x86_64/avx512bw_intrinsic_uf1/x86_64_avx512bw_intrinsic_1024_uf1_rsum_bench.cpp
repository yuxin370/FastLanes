...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// publications/data_parallelized_encodings/fls_generated/x86_64/avx512bw_intrinsic_uf1/x86_64_avx512bw_intrinsic_1024_uf1_rsum_bench.cpp
// ────────────────────────────────────────────────────────
#include "x86_64_avx512bw_intrinsic_1024_uf1_rsum_bench.hpp"
...
static benchmark::BenchmarkReporter::Run bench_0(const uint8_t* transposed8, uint8_t* untransposed8) {
	...
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum::x86_64::avx512bw::rsum(helper::delta_arr_ow8, untransposed8, helper::base_arr_ow8);
	}
...
static benchmark::BenchmarkReporter::Run bench_1(const uint16_t* transposed16, uint16_t* untransposed16) {
	...
	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum::x86_64::avx512bw::rsum(helper::delta_arr_ow16, untransposed16, helper::base_arr_ow16);
	}
...
static benchmark::BenchmarkReporter::Run bench_2(const uint32_t* transposed32, uint32_t* untransposed32) {
	...
	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum::x86_64::avx512bw::rsum(helper::delta_arr_ow32, untransposed32, helper::base_arr_ow32);
	}
...
static benchmark::BenchmarkReporter::Run bench_3(const uint64_t* transposed64, uint64_t* untransposed64) {
	...
	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum::x86_64::avx512bw::rsum(helper::delta_arr_ow64, untransposed64, helper::base_arr_ow64);
	}
...
int main() {
	benchmark::Benchmark benchmark =
	    benchmark::create("x86_64_avx512bw_intrinsic_1024_uf1_rsum")
	        .save()
