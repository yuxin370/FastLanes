...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// publications/data_parallelized_encodings/fls_generated/helper/scalar_av_uf1/helper_scalar_av_1024_uf1_untranspose_bench.cpp
// ────────────────────────────────────────────────────────
#include "helper_scalar_av_1024_uf1_untranspose_bench.hpp"
...
#include "helper_scalar_av_1024_uf1_untranspose_helper.hpp"
#include <iostream>
static benchmark::BenchmarkReporter::Run bench_0(const uint8_t* transposed8, uint8_t* untransposed8) {
	...
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::untranspose::helper::scalar::untranspose_i(transposed8, untransposed8);
	}
...
static benchmark::BenchmarkReporter::Run bench_2(const uint16_t* transposed16, uint16_t* untransposed16) {
	...
	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::untranspose::helper::scalar::untranspose_i(transposed16, untransposed16);
	}
...
static benchmark::BenchmarkReporter::Run bench_4(const uint32_t* transposed32, uint32_t* untransposed32) {
	...
	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::untranspose::helper::scalar::untranspose_i(transposed32, untransposed32);
	}
...
static benchmark::BenchmarkReporter::Run bench_6(const uint64_t* transposed64, uint64_t* untransposed64) {
	...
	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::untranspose::helper::scalar::untranspose_i(transposed64, untransposed64);
	}
...
int main() {
	benchmark::Benchmark benchmark =
	    benchmark::create("helper_scalar_av_1024_uf1_untranspose")
	        .save()
