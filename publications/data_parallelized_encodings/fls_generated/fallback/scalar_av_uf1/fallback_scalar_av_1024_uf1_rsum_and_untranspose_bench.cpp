...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// publications/data_parallelized_encodings/fls_generated/fallback/scalar_av_uf1/fallback_scalar_av_1024_uf1_rsum_and_untranspose_bench.cpp
// ────────────────────────────────────────────────────────
#include "fallback_scalar_av_1024_uf1_rsum_and_untranspose_bench.hpp"
...
static benchmark::BenchmarkReporter::Run bench_0(const uint8_t* transposed8, uint8_t* untransposed8) {
	...
	std::string benchmark_name = "rsum_and_untranspose_8";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum_and_untranspose::fallback::scalar::rsum_and_untranspose(
		    helper::delta_arr_ow8, untransposed8, helper::base_arr_ow8);
	}
...
static benchmark::BenchmarkReporter::Run bench_1(const uint16_t* transposed16, uint16_t* untransposed16) {
	...
	std::string benchmark_name = "rsum_and_untranspose_16";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum_and_untranspose::fallback::scalar::rsum_and_untranspose(
		    helper::delta_arr_ow16, untransposed16, helper::base_arr_ow16);
	}
...
static benchmark::BenchmarkReporter::Run bench_2(const uint32_t* transposed32, uint32_t* untransposed32) {
	...
	std::string benchmark_name = "rsum_and_untranspose_32";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum_and_untranspose::fallback::scalar::rsum_and_untranspose(
		    helper::delta_arr_ow32, untransposed32, helper::base_arr_ow32);
	}
...
static benchmark::BenchmarkReporter::Run bench_3(const uint64_t* transposed64, uint64_t* untransposed64) {
	...
	std::string benchmark_name = "rsum_and_untranspose_64";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::rsum_and_untranspose::fallback::scalar::rsum_and_untranspose(
		    helper::delta_arr_ow64, untransposed64, helper::base_arr_ow64);
	}
...
int main() {
	benchmark::Benchmark benchmark =
	    benchmark::create("fallback_scalar_av_1024_uf1_rsum_and_untranspose")
	        .save()
