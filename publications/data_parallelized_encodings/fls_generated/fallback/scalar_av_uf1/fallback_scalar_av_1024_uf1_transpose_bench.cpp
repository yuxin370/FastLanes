...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// publications/data_parallelized_encodings/fls_generated/fallback/scalar_av_uf1/fallback_scalar_av_1024_uf1_transpose_bench.cpp
// ────────────────────────────────────────────────────────
#include "fallback_scalar_av_1024_uf1_transpose_bench.hpp"
#include "fallback_scalar_av_1024_uf1_transpose_helper.hpp"
...
static benchmark::BenchmarkReporter::Run bench_0(const uint8_t* transposed8, uint8_t* untransposed8) {
	...
	std::string benchmark_name = "transpose_i_8";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_i(transposed8, untransposed8);
	}
...
static benchmark::BenchmarkReporter::Run bench_1(const uint8_t* transposed8, uint8_t* untransposed8) {
	...
	std::string benchmark_name = "transpose_o_8";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_o(transposed8, untransposed8);
	}
...
static benchmark::BenchmarkReporter::Run bench_2(const uint16_t* transposed16, uint16_t* untransposed16) {
	...
	std::string benchmark_name = "transpose_i_16";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_i(transposed16, untransposed16);
	}
...
static benchmark::BenchmarkReporter::Run bench_3(const uint16_t* transposed16, uint16_t* untransposed16) {
	...
	std::string benchmark_name = "transpose_o_16";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_o(transposed16, untransposed16);
	}
...
static benchmark::BenchmarkReporter::Run bench_4(const uint32_t* transposed32, uint32_t* untransposed32) {
	...
	std::string benchmark_name = "transpose_i_32";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_i(transposed32, untransposed32);
	}
...
static benchmark::BenchmarkReporter::Run bench_5(const uint32_t* transposed32, uint32_t* untransposed32) {
	...
	std::string benchmark_name = "transpose_o_32";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_o(transposed32, untransposed32);
	}
...
static benchmark::BenchmarkReporter::Run bench_6(const uint64_t* transposed64, uint64_t* untransposed64) {
	...
	std::string benchmark_name = "transpose_i_64";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_i(transposed64, untransposed64);
	}
...
static benchmark::BenchmarkReporter::Run bench_7(const uint64_t* transposed64, uint64_t* untransposed64) {
	...
	std::string benchmark_name = "transpose_o_64";

	uint64_t cycles = benchmark::cycleclock::Now();
	for (uint64_t i = 0; i < iterations; ++i) {
		generated::transpose::fallback::scalar::transpose_o(transposed64, untransposed64);
	}
...
void benchmark_all(benchmark::Benchmark& benchmark) {
	const auto* transposed8    = new uint8_t[1024]();
	const auto* transposed16   = new uint16_t[1024]();
	const auto* transposed32   = new uint32_t[1024]();
	const auto* transposed64   = new uint64_t[1024]();
	auto*       untransposed8  = new uint8_t[1024]();
	auto*       untransposed16 = new uint16_t[1024]();
	auto*       untransposed32 = new uint32_t[1024]();
	auto*       untransposed64 = new uint64_t[1024]();
...
int main() {
	benchmark::Benchmark benchmark =
	    benchmark::create("fallback_scalar_av_1024_uf1_transpose")
	        .save()
