...
`galp.benchmarks.coefficient_mask_evaluator` and the published training
schedule from `galp.benchmarks.training_pls`. It does not
...
python -m galp.benchmarks.dct_retokenization.train_superpatch --help
python -m galp.benchmarks.dct_retokenization.benchmark_model --help
python -m galp.benchmarks.dct_retokenization.summarize_results --help
```

Frozen experiment contracts live in `configs/`. The small unit suite is:

```bash
python -m unittest discover -s galp/benchmarks/dct_retokenization/tests
```
