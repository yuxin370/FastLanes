...
_TRAINING_RUNTIME_EXTERNAL_PATHS = frozenset(
    {
        ...
        "galp/benchmarks/training_pls/report_equal_image_performance.py",
        "galp/benchmarks/common.py",
...
def code_version(repo_root: Path) -> dict[str, Any]:
    ...
    core_paths = sorted(
        path
        for path in (
            repo_root / "galp/benchmarks/training_pls"
        ).rglob("*")
        if path.is_file()
    ...
    for extra in (
        repo_root / "galp/benchmarks/training_pls/tests/test_training_pls.py",
        *(
            repo_root / relative
...
def _is_training_runtime_source(relative_path: str) -> bool:
    prefix = "galp/benchmarks/training_pls/"
...
def blocking_code_identity(version: Mapping[str, Any]) -> dict[str, str]:
    ...
    if scope != TRAINING_SOURCE_SCOPE or not isinstance(source_hash, str):
        raise ValueError(
            ...
            "regenerate it with galp.benchmarks.training_pls.run_matrix"
        )
    return {
