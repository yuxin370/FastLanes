...
def assert_recipe_overrides(
    *,
    ...
    if compile_mode not in ("default", "reduce-overhead"):
        raise ValueError(f"unsupported model compile mode: {compile_mode!r}")
    contract["execution"]["model_compile"]["mode"] = compile_mode
    contract["recipe_hash"] = sha256_json(
        {key: value for key, value in contract.items() if key != "recipe_hash"}
    )
    return contract
