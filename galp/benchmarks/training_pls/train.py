...
RGB_BENCHMARK_ROOT = Path(__file__).resolve().parents[1] / "system_rgbnomore"

from galp.benchmarks.system_rgbnomore.training.artifacts import nested_state_sha256, tensor_state_sha256  # noqa: E402
from galp.benchmarks.system_rgbnomore.training.direct_dct_reader import DirectDctTrainingReader  # noqa: E402
...
from galp.benchmarks.system_rgbnomore.training.sample_order import SampleIdentity  # noqa: E402
...
def compile_published_model(
    ...
    if configuration["mode"] == "reduce-overhead":
...
def _run_dct_model_only_calibration(
    *,
    ...
    calibration = run_model_only_calibration(
        ...
        cuda_graphs=recipe["execution"]["model_compile"]["mode"] == "reduce-overhead",
    )
    calibration.update(
        condition_hash=contract["condition_hash"],
        initial_model_hash=initial_model_hash,
            ...
            for chunk_begin in range(
                ...
                try:
                    for window_begin in range(0, len(identities), 16):
                        ...
                        optimizer.zero_grad(set_to_none=args.compile_mode != "reduce-overhead")
                        learning_rate = scheduler.prepare_next_update()
                        audit_decision = decision_for_next_update(
                            resolved_audit_policy, global_update
                        )
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-manifest", type=Path, required=True,
        help="resolved run manifest generated by galp.benchmarks.training_pls.run_matrix",
    )
    parser.add_argument(
        "--condition",
        required=True,
