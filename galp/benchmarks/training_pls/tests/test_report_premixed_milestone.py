...
from galp.benchmarks.training_pls.matrix import CORE_CONDITION_IDS
from galp.benchmarks.training_pls.report_premixed_milestone import (
    EXPECTED_POLICIES,
    effect_rows,
    generate_report,
...
class PremixedMilestoneReportTests(unittest.TestCase):
    def make_runs(self, root: Path, *, omit_validation: str | None = None) -> dict[str, Path]:
        ...
        for index, condition in enumerate(CORE_CONDITION_IDS):
            ...
            (run / "run_manifest.json").write_text(json.dumps(contract) + "\n", encoding="utf-8")
