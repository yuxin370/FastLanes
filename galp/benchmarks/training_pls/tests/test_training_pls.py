...
from galp.benchmarks.training_pls.core_schedule import (
    crop_key,
...
from galp.benchmarks.training_pls.layout import (
    LayoutMapping,
    create_layout_plan,
    load_layout_mapping,
...
from galp.benchmarks.training_pls.matrix import (
...
from galp.benchmarks.training_pls.report import aggregate, mean_ci, normalized_auc
...
from galp.benchmarks.training_pls.run_matrix import _parse_args as _parse_matrix_args
from galp.benchmarks.training_pls.run_matrix import _seed_devices, _write_or_validate_contract, build_plan
...
class CoreScheduleTests(unittest.TestCase):
    ...
    def test_grouped_randaugment_is_exactly_scalar_equivalent(self) -> None:
        rgbnomore_root = DEFAULT_RGBNOMORE_ROOT
...
class RecipeAndContractTests(unittest.TestCase):
    ...
    def test_native_physical_epoch_consumes_native_order_and_updates_model(self) -> None:
        ...
        def run_epoch(mode):
            with tempfile.TemporaryDirectory() as temporary:
                ...
                with mock.patch("torch.cuda.synchronize"), mock.patch("torch.compiler.cudagraph_mark_step_begin") as mark_step:
                    result = _train_native_physical_epoch(
                        ...
                        recipe={
                            "execution": {"model_compile": {"mode": mode}},
                            "training": {
                ...
                self.assertEqual(mark_step.call_count, 2 if mode == "reduce-overhead" else 0)
                return [parameter.detach().clone() for parameter in model.parameters()]

        default = run_epoch("default")
        graph = run_epoch("reduce-overhead")
        for expected, actual in zip(default, graph):
    ...
    def test_graph_recipe_preserves_training_math_and_has_distinct_identity(self) -> None:
        default = assert_recipe_overrides(recipe=None)
        graph = assert_recipe_overrides(recipe=None, compile_mode="reduce-overhead")
        self.assertNotEqual(default["recipe_hash"], graph["recipe_hash"])
        self.assertEqual(graph["recipe_hash"], sha256_json(
            {key: value for key, value in graph.items() if key != "recipe_hash"}
        ))
        graph["execution"]["model_compile"]["mode"] = "default"
        graph["recipe_hash"] = default["recipe_hash"]
        self.assertEqual(graph, default)
    ...
    def test_existing_run_plan_uses_training_identity(self) -> None:
        planned = {
            "code_version": {
                "commit": "planned",
                "training_source_scope": TRAINING_SOURCE_SCOPE,
            ...
            "train_manifest_hash": "dataset-stable",
        }
        current = {**planned, "code_version": {**planned["code_version"], "commit": "current"}}
        with tempfile.TemporaryDirectory() as temporary:
            path = Path(temporary) / "run_manifest.json"
            _write_or_validate_contract(path, planned)
            original = path.read_bytes()
            _write_or_validate_contract(path, current)
            self.assertEqual(path.read_bytes(), original)
            with self.assertRaisesRegex(ValueError, "existing run contract differs"):
                _write_or_validate_contract(path, {**current, "train_manifest_hash": "changed"})
            current["code_version"]["training_runtime_source_tree_sha256"] = "runtime-changed"
            with self.assertRaisesRegex(ValueError, "existing run contract differs"):
                _write_or_validate_contract(path, current)
    ...
    def test_contract_validation_treats_repository_provenance_as_non_blocking(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch(
                "galp.benchmarks.training_pls.train.code_version", return_value=current_code
            ):
            ...
            with mock.patch(
                "galp.benchmarks.training_pls.train.code_version", return_value=changed_runtime
            ):
                with self.assertRaisesRegex(
    ...
    def test_matrix_plan_writes_canonical_run_manifest_and_forwards_pause(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch(
                    "galp.benchmarks.training_pls.run_matrix.load_layout_plan",
                    return_value=layout_plan,
                ),
                mock.patch(
                    "galp.benchmarks.training_pls.run_matrix.load_layout_mapping",
                    return_value=mapping,
                ),
                mock.patch(
                    "galp.benchmarks.training_pls.run_matrix._initial_model_hashes",
                    return_value={11997733: "initial-model"},
                ),
                mock.patch(
                    "galp.benchmarks.training_pls.run_matrix.code_version", return_value=version
                ),
                mock.patch(
                    "galp.benchmarks.training_pls.run_matrix._environment", return_value={}
                ),
            ):
