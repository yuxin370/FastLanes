...
from galp.benchmarks.system_dct_major.training_pls.contracts import (
    NATIVE_PHYSICAL_BACKEND,
    STANDARD_DCT_BACKEND,
    blocking_code_identity,
    condition_identity_hash,
)
from galp.benchmarks.system_dct_major.training_pls.report_convergence_reference import (
    PAIRED_IDENTITY_FIELDS,
)
from galp.benchmarks.system_dct_major.training_pls.recipe import sha256_json
...
def _read_pls_contract(run: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        path = run / "condition_contract.json"
...
def build_report(
    *,
    ...
    standard_model_ids = {
        str(observed.get("model", {}).get("model_id", ""))
        for observed in contracts.values()
    }
    native_model_id = str(native_condition.get("model_id", ""))
    recorded_model_ids = {value for value in standard_model_ids | {native_model_id} if value}
    if recorded_model_ids and (
        standard_model_ids != {native_model_id} or not native_model_id
    ):
        raise ValueError(
    ...
    if not native_model_id:
        native_model_id = "legacy-unspecified"
...
def _write_outputs(
    ...
    lines = [
        ...
        "",
        f"Model: `{report.get('model_id', 'legacy-unspecified')}`; precision: "
        f"`{report.get('precision', 'legacy-unspecified')}`.",
        "",
