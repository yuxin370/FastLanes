...
def main():
    ...
    text += ['', 'MobileNetV2 DCT-24 原双 float pool 短测进程峰值 22550 MiB，原位整理后为 13142 MiB，降低约 41.7%。'
        ...
        f'训练入口：`{HERE/"cnn_training_baselines.sh"}`。', '',
        'B6 命令：`bash galp/benchmarks/dct_models/cnn_training_baselines.sh full cnn_training "mobilenet24:native mobilenet32:native resnet24:native resnet64:native"`。', '',
        '标准 baseline 命令：`bash galp/benchmarks/dct_models/cnn_training_baselines.sh full rtx4090_cnn_training_baselines_20260914`。', '',
