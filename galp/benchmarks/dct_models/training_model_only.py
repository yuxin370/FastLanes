...
from galp.benchmarks.dct_models.evaluate import Inputs
...
def main():
    ...
    # Match the from-scratch initialization used by training_pls.py.
    net = rgb.model() if args.domain == "rgb" else B.model()
    torch.manual_seed(11997733)
    ...
    if args.domain == "dct":
        # Match the dense-target cross-entropy path used by B6 Mixup.
        targets = torch.nn.functional.one_hot(targets, num_classes=1000).float()
    ...
    compile_options = {k: args.compile_mode if k == "mode" else v
                       for k, v in recipe["execution"]["model_compile"].items() if k != "enabled"}
    execution_model = torch.compile(net, **compile_options)
    ...
    result = run_model_only_calibration(
        ...
        precision=args.precision, cuda_graphs=args.compile_mode == "reduce-overhead")
    ...
    result.update(
        ...
        initialization="from scratch; official CNN layers reset with seed 11997733; calibration updates discarded",
        ...
        model_compile=compile_options,
        target_format="dense one-hot" if args.domain == "dct" else "class indices",
