...
> 这里的 N 目标副本仅用于历史复现，已隔离至 `backup/offline_model_input/`；正式在线系统数据见[统一数据清单](../../docs/DATASETS.md)。
...
PY
$PYTHON -m unittest -v galp.benchmarks.dct_models.tests.test_efun
...
$PYTHON "$EXP/efun.py" generate --count 50000 --layout block-major \
  ...
  --output-dir "galp/data/compressed/backup/offline_model_input/imagenet512_val_efun28_block_major"
build/galp/tools/jpeg_dct/galp_block_major_access_tool \
  "galp/data/compressed/backup/offline_model_input/imagenet512_val_efun28_block_major/manifest.bin" --output-dir "galp/data/compressed/backup/offline_model_input/imagenet512_val_efun28_block_major/access" \
  --output-json "galp/data/compressed/backup/offline_model_input/imagenet512_val_efun28_block_major/access.json"
$PYTHON "$EXP/efun.py" validate "galp/data/compressed/backup/offline_model_input/imagenet512_val_efun28_block_major"
...
for variant in grid_off projected_off projected_on; do
  ...
  $PYTHON "$EXP/efun.py" evaluate_shards --count 50000 --batch-size 64 \
    --new-data "galp/data/compressed/backup/offline_model_input/imagenet512_val_efun28_block_major" --output-layout "$layout" --pushdown "$mode" \
...
bash galp/benchmarks/dct_models/efun_profile.sh nsys_20260915
```
...
```bash
EFUN_RGB_TEST_CUDA=1 $PYTHON -m unittest -v galp.benchmarks.dct_models.tests.test_efun
...
```bash
PHYSICAL=/home/tangyuxin/gfastlanes/FastLanes/galp/data/compressed/imagenet512_train_block_major_premixed
...
for spec in jpeg:A0 native:A0 native:B6; do
  ...
  $PYTHON "$EXP/efun.py" training_pls --manifest "${INPUTS[0]}" \
    ...
    --epochs 2 --validation-data "galp/data/compressed/backup/offline_model_input/imagenet512_val_efun28_block_major" \
