...
def validate(net, count, root):
    ...
    from galp.benchmarks.dct_models.evaluate_shards import native_options, parse_manifest
...
def main():
    ...
    # Historical N validation uses archived model targets, not formal system source data.
    validation_name = ("imagenet512_val_mobilenet112_block_major" if B.PROFILE.startswith("mobilenet")
                       else "imagenet512_val_resnet56_block_major")
    p.add_argument("--validation-data", type=Path,
                   default=B.REPO / "galp/data/compressed/backup/offline_model_input" / validation_name)
    ...
    if rgb_backend:
        from galp.benchmarks.dct_models.training_rgb import RgbTrainingPipeline
