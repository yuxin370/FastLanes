...
template <>
uint64_t* decompress_column<uint64_t, flsgpu::device::CONSTANTColumn<uint64_t>>(
    const flsgpu::device::CONSTANTColumn<uint64_t> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 1) {
		return kernels::host::decompress_column<
		    ...
		    1,
		    1,
		    flsgpu::device::CONSTANTDecompressor<uint64_t, 1, 1, flsgpu::device::CONSTANTColumn<uint64_t>>,
		    flsgpu::device::CONSTANTColumn<uint64_t>>(column, n_samples);
	}
	if (unpack_n_vectors == 4 && unpack_n_values == 1) {
		return kernels::host::decompress_column<
		    ...
		    4,
		    1,
		    flsgpu::device::CONSTANTDecompressor<uint64_t, 4, 1, flsgpu::device::CONSTANTColumn<uint64_t>>,
		    flsgpu::device::CONSTANTColumn<uint64_t>>(column, n_samples);
	}
