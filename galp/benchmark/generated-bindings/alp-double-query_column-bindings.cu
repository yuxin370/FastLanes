...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// galp/benchmark/generated-bindings/alp-double-query_column-bindings.cu
// ────────────────────────────────────────────────────────
...
template <>
bool query_column<double, galp::codec::device::ALPColumn<double>>(const galp::codec::device::ALPColumn<double> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 1 && unpacker == galp::format::Unpacker::StatefulBranchless &&
		...
		return galp::kernels::host::query_column<
		    ...
		    galp::codec::device::ALPColumn<double>>(column, magic_value, n_samples);
	}
	if (unpack_n_vectors == 1 && unpack_n_values == 1 && unpacker == galp::format::Unpacker::StatefulBranchless &&
	...
	throw std::invalid_argument("Could not find correct binding in query_column ALP<double>");
}
