...
template <>
uint64_t* decompress_column<uint64_t, galp::codec::device::DICTFFORColumn<uint64_t>>(
    const galp::codec::device::DICTFFORColumn<uint64_t> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 1 && unpacker == galp::format::Unpacker::Dummy &&
		...
		return galp::kernels::host::decompress_column<
		    ...
		    galp::codec::device::DICTDecompressor<
		        ...
		        galp::codec::device::DICTFFORColumn<uint64_t>>,
		    galp::codec::device::DICTFFORColumn<uint64_t>>(column, n_samples);
	}
