...
template <>
bool query_column<uint64_t, galp::codec::device::FFORColumn<uint64_t>>(const galp::codec::device::FFORColumn<uint64_t> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 1 && unpacker == galp::format::Unpacker::Dummy &&
		...
		return galp::kernels::host::query_column<
		    ...
		    galp::codec::device::FFORColumn<uint64_t>>(column, magic_value, n_samples);
	}
