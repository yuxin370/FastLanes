...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// galp/benchmark/generated-bindings/alp-float-query_column-bindings.cu
// ────────────────────────────────────────────────────────
...
template <>
bool query_column<float, galp::codec::device::ALPColumn<float>>(const galp::codec::device::ALPColumn<float> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 32 && unpacker == galp::format::Unpacker::OldFls &&
		...
		return galp::kernels::host::query_column<
		    ...
		    galp::codec::device::ALPColumn<float>>(column, magic_value, n_samples);
	}
	if (unpack_n_vectors == 1 && unpack_n_values == 32 && unpacker == galp::format::Unpacker::OldFls &&
	...
	throw std::invalid_argument("Could not find correct binding in query_column ALP<float>");
}
