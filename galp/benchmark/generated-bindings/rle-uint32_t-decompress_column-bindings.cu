...
template <>
uint32_t* decompress_column<uint32_t, flsgpu::device::RLEColumn<uint32_t, uint32_t>>(
    const flsgpu::device::RLEColumn<uint32_t, uint32_t> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 1 && unpacker == enums::Unpacker::Dummy &&
		...
		return kernels::host::decompress_column<
		    ...
		    flsgpu::device::RLEDecompressor<
		        ...
		        flsgpu::device::DummyRLEExpander<uint32_t, uint32_t, 1, 1>,
		        flsgpu::device::RLEColumn<uint32_t, uint32_t>>,
		    flsgpu::device::RLEColumn<uint32_t, uint32_t>>(column, n_samples);
	}
	if (unpack_n_vectors == 1 && unpack_n_values == 32 && unpacker == enums::Unpacker::OldFls &&
		...
		return kernels::host::decompress_column<
		    ...
		    flsgpu::device::RLEDecompressor<
		        ...
		        flsgpu::device::DummyRLEExpander<uint32_t, uint32_t, 1, 32>,
		        flsgpu::device::RLEColumn<uint32_t, uint32_t>>,
		    flsgpu::device::RLEColumn<uint32_t, uint32_t>>(column, n_samples);
	}
	...
	if (unpack_n_vectors == 4 && unpack_n_values == 1 && unpacker == enums::Unpacker::Dummy &&
		...
		return kernels::host::decompress_column<
		    ...
		    flsgpu::device::RLEDecompressor<
		        ...
		        flsgpu::device::DummyRLEExpander<uint32_t, uint32_t, 4, 1>,
		        flsgpu::device::RLEColumn<uint32_t, uint32_t>>,
		    flsgpu::device::RLEColumn<uint32_t, uint32_t>>(column, n_samples);
	}
