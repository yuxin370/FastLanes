...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// galp/benchmark/generated-bindings/rle-uint64_t-decompress_column-bindings.cu
// ────────────────────────────────────────────────────────
...
template <>
uint64_t* decompress_column<uint64_t, flsgpu::device::RLEColumn<uint64_t, uint64_t>>(
    const flsgpu::device::RLEColumn<uint64_t, uint64_t> column,
	...
	if (unpack_n_vectors == 1 && unpack_n_values == 1 && unpacker == enums::Unpacker::Dummy &&
		...
		return kernels::host::decompress_column<
		    ...
		    1,
		    1,
		    flsgpu::device::RLEDecompressor<
		        uint64_t,
		        uint64_t,
		        1,
		        1,
		        flsgpu::device::BitUnpackerDummy<uint64_t, 1, 1, flsgpu::device::FFORFunctor<uint64_t, 1>>,
		        flsgpu::device::DummyRLEExpander<uint64_t, uint64_t, 1, 1>,
		        flsgpu::device::RLEColumn<uint64_t, uint64_t>>,
		    flsgpu::device::RLEColumn<uint64_t, uint64_t>>(column, n_samples);
	}
	...
	if (unpack_n_vectors == 4 && unpack_n_values == 1 && unpacker == enums::Unpacker::Dummy &&
		...
		return kernels::host::decompress_column<
		    ...
		    flsgpu::device::RLEDecompressor<
		        ...
		        flsgpu::device::DummyRLEExpander<uint64_t, uint64_t, 4, 1>,
		        flsgpu::device::RLEColumn<uint64_t, uint64_t>>,
		    flsgpu::device::RLEColumn<uint64_t, uint64_t>>(column, n_samples);
	}
	...
	throw std::invalid_argument("Could not find correct binding in decompress_column RLE<uint64_t>");
}
