...
struct TableBenchmarkResult {
	...
	double run_submit_wall_ms           = 0.0; // host wall around runtime::run_workset_async
	double wait_workset_wall_ms         = 0.0; // host wall around runtime::wait_workset_async
	double event_sync_wall_ms           = 0.0; // host wall inside cudaEventSynchronize(stop)
	double pre_kernel_event_ms          = 0.0; // compute-stream event time before the timed kernel span
	double warmup_wall_ms               = 0.0; // first-run warmup launch + synchronize wall time
	double timing_event_create_ms       = 0.0; // cuda timing-event creation in run_workset_async
	double timing_event_destroy_ms      = 0.0; // cuda timing-event destroy in release_workset
	...
	size_t   wait_ready_chunks              = 0;
	size_t   wait_blocking_chunks           = 0;
