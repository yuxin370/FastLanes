...
下面是实际生成任务记录，生成 worker 时间和编码时间为跨任务累计，不与wall相加。''')
    generation=[]
    for label, d in [('ResNet：3×64×56×56','dct_major_dctnet_static64'),('MobileNet：3×64×112×112','dct_major_dctnet_mobilenet32')]:
        g=read(DATA/d/'generation.json'); parts=[read(p) for p in (DATA/d).glob('shard_*.json')]
...
### 10.1 实际路径与运行入口''')
    paths=[('报告对应推理运行',DATA/'runs/dctnet_mobilenet24'/RUN),('完整CSV与原始汇总',SUMMARY),
        ...
        ('56网格目标数据',DATA/'dct_major_dctnet_static64'),('112网格目标数据',DATA/'dct_major_dctnet_mobilenet32'),
        ('源训练premix',Path('/mnt/nvme2/home/tangyuxin/pls-experiments/physical-layout-full-premix-orgseed-20260810/uniform_premix')),
