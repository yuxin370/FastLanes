"""Attribute CNN training CUDA work by launch thread and model NVTX ranges."""
import argparse
import bisect
...
def gap_breakdown(c, start, end, main_tid):
    ...
    for name, a, b, tid in _named_nvtx(c, start, end):
        if tid == main_tid and name.startswith(('detail.', 'model.', 'input.')):
