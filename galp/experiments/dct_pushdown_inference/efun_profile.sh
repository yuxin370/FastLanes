...
for route in jpeg grid_off projected_off projected_on rgb_pytorch rgb_dali; do
  ...
  case "$route" in
    jpeg)
      NAME=dct_reference
      COMMAND=("$PYTHON" "$EXP/efun.py" evaluate --route R --device cuda --workers 16) ;;
    grid_off|projected_off|projected_on)
      NAME=galp_projected
      if [[ "$route" == grid_off ]]; then NAME=galp_native; fi
      COMMAND=("$PYTHON" "$EXP/efun.py" evaluate_shards --new-data "$DATA/dct_major_efun"
        --output-layout "${route%_*}" --pushdown "${route##*_}") ;;
    rgb_pytorch|rgb_dali)
      NAME=$route
      WORKERS=16
      if [[ "$route" == rgb_dali ]]; then WORKERS=4; fi
      COMMAND=("$PYTHON" "$EXP/efun.py" evaluate_rgb --route "${route#rgb_}" --workers "$WORKERS")
  ...
  "$PYTHON" "$EXP/analyze_profiles.py" "$DIR" > "$DIR/analysis.log"
done
...
for backend in jpeg native rgb_pytorch rgb_d2; do
  ...
  capture "$DIR/capture" "$PYTHON" "$EXP/efun.py" training_pls \
  ...
  "$PYTHON" "$EXP/analyze_training_capture.py" "$DIR/capture.sqlite" \
