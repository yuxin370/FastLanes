...
inline void launch_mixed_dispatch(ExecutionWorkset& workset, cudaStream_t stream) {
	const auto*            exprs_i8  = workset.device_batches.template get<int8_t>().d_exprs;
	const auto*            exprs_i16 = workset.device_batches.template get<int16_t>().d_exprs;
	...
	kernels::device::decompress_dispatch_mixed<1, 1, WRITE_OUT>
	    <<<grid, block, 0, stream>>>(exprs_i8, exprs_i16, workset.d_slots, workset.mixed_slots.size());
...
inline AsyncWorksetRun run_workset_async(ExecutionWorkset&      workset,
	...
	if (mixed_dispatch && (workset.d_slots == nullptr || workset.mixed_slots.empty())) {
		if (out_launches) {
			*out_launches = 0;
		}
		return handle;
