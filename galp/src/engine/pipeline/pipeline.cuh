...
// |                      FastLanes                       |
// ────────────────────────────────────────────────────────
// galp/src/engine/pipeline/pipeline.cuh
// ────────────────────────────────────────────────────────
...
#include "engine/table/runner.cuh"
#include <chrono>
...
inline PreparedTableResources prepare_table_resources(const std::filesystem::path& fls_path,
	...
	resources.compute_inflight_chunks =
	    resources.whole_table
	        ...
	        : 1U;
	resources.rowgroup_prefetch_depth = resources.whole_table
	                                        ? detail::choose_rowgroup_prefetch_depth(request.config,
	                                                                                 resources.max_rowgroups_per_chunk,
	                                                                                 resources.compute_inflight_chunks)
	                                        : std::max<size_t>(1, request.config.prefetch_depth);
	observer.on_compute_inflight_chunks(resources.compute_inflight_chunks);
	observer.on_rowgroup_prefetch_depth(resources.rowgroup_prefetch_depth);

	if (resources.whole_table) {
		const size_t active_io_workers    = resources.prefetch_workers;
		const size_t prefetch_depth_slots = resources.rowgroup_prefetch_depth;
		...
		const size_t compute_slots =
		    saturated_mul(resources.compute_inflight_chunks + 1U, resources.max_rowgroups_per_chunk);
		const size_t prefetch_slots    = saturated_add(prefetch_depth_slots, saturated_add(active_io_workers, 2U));
		const size_t pooled_slots      = saturated_add(compute_slots, prefetch_slots);
		...
		if (resources.use_rowgroup_prefetch || warm_slots != 0) {
			...
			if (resources.use_rowgroup_prefetch) {
				if (request.config.max_prefetch_storage_bytes != 0) {
					...
					const size_t depth = resources.rowgroup_prefetch_depth;
...
inline TableData execute_prepared_table_pipeline(PreparedTableResources&                      resources,
	...
	StreamingChunkRing<detail::TableChunkState> pipeline {resources.compute_inflight_chunks};
	...
	const auto submit_chunk = [&](detail::TableChunkState& chunk) {
		...
		observer.on_run_submit(std::chrono::duration<double, std::milli>(run_submit_end - run_submit_start).count(),
		                       chunk.run.timing_event_create_ms,
		                       chunk.run.warmup_wall_ms);
	...
	const auto drain_ready_chunks = [&]() {
		pipeline.drain_ready(
		    [](const detail::TableChunkState& chunk) {
			    return !chunk.submitted || chunk.rowgroups.empty() || runtime::workset_run_is_ready(chunk.run);
		    },
		    consume_chunk);
	};
