    ...
    if (auto* struct_ptr = std::get_if<std::unique_ptr<fastlanes::Struct>>(&variant_elem)) {
        if (*struct_ptr && !(*struct_ptr)->internal_rowgroup.empty()) {
            ...
            for (auto v : channel.AC_values) {
                ...
                try {
                    Attribute::Ingest(physical_column1, std::to_string(v),
            ...
            for (auto v : channel.mix_run_nonzero_values) {
                ...
                try {
                    Attribute::Ingest(physical_column2, std::to_string(v),
            ...
            for (auto& pattern : channel.mix_run_pattern) {
                ...
                try {
                    ...
                    Attribute::Ingest(physical_column3, std::to_string(pattern.zero_count),
                        *cur_rowgroup->m_descriptor.m_column_descriptors[0]->children[3]);
                    Attribute::Ingest(physical_column3, std::to_string(pattern.nonzero_count),
                        *cur_rowgroup->m_descriptor.m_column_descriptors[0]->children[3]);
                } catch (const std::exception& e) {
...
// up<Table> DctChannelReader::ReadMultipleChannels(const std::vector<ProcessedDCTChannel>& channels, const Connection& connection) 
// {
//     auto table = make_unique<Table>(connection);
...
// void DctChannelReader::AddChannelToTable(Table& table, const ProcessedDCTChannel& channel, const Connection& connection) 
// {
