...
std::vector<std::vector<std::string>> to_numpy_dct(fastlanes::TableReader& self) {
        ...
        for (n_t row_idx = 0; row_idx < n_rows; row_idx++) {
            for (n_t col_idx = 0; col_idx < n_cols; col_idx++) {
                ...
                result[old_size + row_idx][col_idx] =
                    Attribute::ToStr(col, row_idx, desc.m_column_descriptors[col_idx]->data_type);
            }
        }
...
void bind_table_reader(py::module_& m) {
	...
	cls.def(
		...
		// .def("to_numpy_dct", &fastlanes::TableReader::fls_to_numpy_dct, 
		.def("to_numpy_dct", &fastlanes::to_numpy_dct, "Convert FLS file to numpy array")
	    .def("__repr__", [](const fastlanes::TableReader&) { return "<fastlanes.TableReader>"; })
	    .def("__dir__", []() {
		    return std::vector<std::string> {
		        "to_csv","to_numpy_dct", "__repr__", "__dir__"
