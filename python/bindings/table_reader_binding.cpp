...
py::array_t<uint8_t> to_numpy_rgb(fastlanes::TableReader& self, const char* path, bool channel_first = false) {
	...
	//             throw std::runtime_error("Image " + std::to_string(img) + " channel " + std::to_string(c) + " height
	//             mismatch");
	//         }
	//         for (size_t h = 0; h < height; ++h) {
	//             if (rgb_out[img][c][h].size() != width) {
	//                 throw std::runtime_error("Image " + std::to_string(img) + " channel " + std::to_string(c) + " row
	//                 " + std::to_string(h) + " width mismatch");
	//             }
	//         }
	//     }
	// }
	...
	if (!channel_first) {
		// NHWC: (N, H, W, 3)
		result       = py::array_t<uint8_t>({static_cast<ssize_t>(N),
